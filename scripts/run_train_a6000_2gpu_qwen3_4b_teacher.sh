...
STUDENT_MODEL_DIR=/home/shenyl/hf/model/Qwen/Qwen3-1.7B
TEACHER_MODEL_DIR=/home/shenyl/hf/model/Qwen/Qwen3-4B

export CUDA_DEVICE_ORDER=PCI_BUS_ID
# Default to the two A6000 cards discussed earlier. Override if needed.
...
TRAIN_PROMPT_BSZ=32
# With VAL_REPEAT_N=16 and rollout DP=2, val_batch_size=8 gives about
# 64 requests per student engine, matching the default max_num_seqs below.
...
ROLLOUT_TP=1
ROLLOUT_DP=${ROLLOUT_DP:-${STUDENT_WORLD_SIZE}}

# The 4B teacher is sharded over both A6000 cards.
...
# 4B teacher has a larger weight footprint than 1.7B. Keep teacher memory
# conservative on 48GB A6000 and tune upward only after checking peak memory.
ROLLOUT_GPU_MEMORY_UTILIZATION=${ROLLOUT_GPU_MEMORY_UTILIZATION:-0.38}
TEACHER_GPU_MEMORY_UTILIZATION=${TEACHER_GPU_MEMORY_UTILIZATION:-0.16}

LORA_RANK=32
LORA_ALPHA=64

MODEL_TAG="$(basename "${STUDENT_MODEL_DIR}" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/_/g; s/_\\+/_/g; s/^_//; s/_$//')"
TEACHER_TAG="$(basename "${TEACHER_MODEL_DIR}" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/_/g; s/_\\+/_/g; s/^_//; s/_$//')"
LOSS_TAG="$(echo "${DISTILLATION_LOSS_MODE}" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/_/g; s/_\\+/_/g; s/^_//; s/_$//')"
RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
EXP_NAME="${MODEL_TAG}_${DISTILLATION_LOSS_MODE}_${DISTILLATION_TOPK}_${USE_POLICY_GRADIENT}_a6000x2_teacher_${TEACHER_TAG}_baseline_${LOSS_TAG}_official_eval_bs${TRAIN_PROMPT_BSZ}_mb${STUDENT_MICRO_BATCH_SIZE_PER_GPU}_ws${STUDENT_WORLD_SIZE}_seq${ROLLOUT_MAX_NUM_SEQS}_${RUN_TIMESTAMP}"
...
export VERL_ZMQ_NAMESPACE="u$(id -u)-$(date +%s)-official-eval-a6000x2-qwen3-4b-teacher"
...
MODEL=(
    actor_rollout_ref.model.path="${STUDENT_MODEL_DIR}"
...
DISTILLATION=(
    ...
    distillation.teacher_model.model_path="${TEACHER_MODEL_DIR}"
